...
public class EndGenerator extends NoiseChunkGenerator {
    ...
    public int getHeight(int x, int z, Heightmap.Type heightmap, HeightLimitView world, NoiseConfig noiseConfig) {
        for(int i = Math.min(endList.size(), world.getTopY()) - 1; i >= 0; --i) {
            BlockState blockState = endList.get(i).getDefaultState();
    ...
    public VerticalBlockSample getColumnSample(int x, int z, HeightLimitView world, NoiseConfig noiseConfig) {
        return new VerticalBlockSample(world.getBottomY(), endList.stream().limit(world.getHeight()).map((state) -> state == null ? Blocks.AIR.getDefaultState() : state.getDefaultState()).toArray(BlockState[]::new));
    }
    @Override
    public CompletableFuture<Chunk> populateNoise(
        ...
        for (int i = 0; i < Math.min(chunk.getHeight(), endList.size()); ++i) {
            BlockState blockState = endList.get(i).getDefaultState();
            for (int k = 0; k < 16; ++k) {
                for (int l = 0; l < 16; ++l) {
                    mutable.set(k, i, l);
                    Biome currentBiome = chunk.getBiomeForNoiseGen(k >> 2, i, l >> 2).value();
                    if (currentBiome.equals(biomeRegistry.get(BiomeKeys.END_HIGHLANDS))) {
                        blockState = highlandList.get(i).getDefaultState();
                    } else if (currentBiome.equals(biomeRegistry.get(BiomeKeys.END_BARRENS))) {
                        blockState = barrenList.get(i).getDefaultState();
                    } else if (currentBiome.equals(biomeRegistry.get(BiomeKeys.END_MIDLANDS))) {
                        blockState = midlandList.get(i).getDefaultState();
                    } else if (currentBiome.equals(biomeRegistry.get(BiomeKeys.SMALL_END_ISLANDS))) {
                        blockState = smallList.get(i).getDefaultState();
                    } else if (currentBiome.equals(biomeRegistry.get(BiomeKeys.THE_END))) {
                        blockState = endList.get(i).getDefaultState();
                    }
